...
[고유명사 표기 규칙]
- 아티스트·멤버·그룹 이름은 가능한 한 영문(라틴) 표기를 사용하세요.
- 우선순위: (1) 소스 데이터에 영문 표기 있음 → 그대로 사용, (2) 널리 알려진 공식 영문명 있음 → 사용, (3) 영문명 불확실 → 자연스럽게 로마자화 (예: 지수 → Jisoo, 재현 → Jaehyun)
- 정말 영문 표기를 알 수 없는 경우에만 한글 유지
- 학교명·장소명·기관명은 한글로 두지 말고 번역하거나 로마자 표기를 사용하세요.`
...
PROPER NOUN RULE:
- K-pop artist / member / group names → use English (Latin) spelling whenever possible
- Priority order: (1) Latin spelling in source data → use verbatim, (2) widely known official English name from training → use it, (3) no known English name → romanize naturally (e.g. 지수 → Jisoo, 재현 → Jaehyun)
- Only fall back to Korean Hangul if you truly have no idea of the English form
- School names, place names, institutions → translate to Vietnamese or use romanized form. NEVER leave them in Korean Hangul in Vi fields.
...
- Body slides: one fact per slide, bodyVi strictly 2 sentences max, each sentence under 15 Vietnamese words (total body under 30 words) (+ Ko equivalents)
