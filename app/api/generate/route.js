...
import { scanSourceForNames, resolveArtistName } from '@/lib/resolveArtistName'
...
[고유명사 표기 규칙 — 절대 준수]
⚠️  당신은 모든 K-pop 아티스트의 공식 영문 표기를 정확히 알지 못합니다.
    학습 데이터에 틀린 로마자 표기가 포함되어 있을 수 있습니다. 추측하지 마세요.

- 아티스트·멤버·그룹 이름은 소스 데이터에 있는 표기를 그대로 복사하세요.
- 소스에 한국어(한글)로만 나온 경우 → 한글 그대로 유지하세요. 절대 영문으로 변환하지 마세요.
- 소스에 영문 표기가 이미 있는 경우 → 그 표기를 그대로 사용하세요.
  ❌ 키나 → "Keyner" / 재현 → "Jae-hyeon" (추측 음역 — 절대 금지)
  ✅ 소스에 "키나"만 있음 → "키나" 그대로 사용
  ✅ 소스에 "Keena" 있음 → "Keena" 그대로 사용`

async function buildContext({ video, captions, news, artist, topic, type, hook, reason, keywords, articleText, articleSite, articleUrl }) {
  const lines = []
...
태그: ${video.tags.join(', ')}`)
  ...
  // ── 아티스트 영문명 해석 ──────────────────────────────────────
  // 소스 전체 텍스트에서 알려진 이름 스캔
  const sourceText = [artist, topic, keywords, articleText, captions?.text, video?.title, video?.description]
    .filter(Boolean).join(' ')
  const nameMap = scanSourceForNames(sourceText)

  // 명시된 artist 파라미터가 한글이고 아직 매핑에 없으면 Gemini로 추가 조회
  if (artist && /[가-힣]/.test(artist) && !nameMap[artist]) {
    const enName = await resolveArtistName(artist)
    if (enName) nameMap[artist] = enName
  }

  if (Object.keys(nameMap).length > 0) {
    const nameRef = Object.entries(nameMap)
      .map(([ko, en]) => `  ${ko} → "${en}"`)
      .join('\n')
    lines.push(`[공식 영문명 참조 — Vi 필드에서 반드시 이 표기 사용]\n${nameRef}`)
  }
...
- Do NOT put Korean text (Hangul) in any Vi field. Ever.

PROPER NOUN RULE — HARD BLOCK:
⚠️  You do NOT reliably know the official English names of all K-pop artists.
    Your training data contains WRONG romanizations. Do not trust it for names.

- K-pop artist / member / group names → copy EXACTLY from the source data as-is
- If the source has the name in Korean Hangul only → keep it in Hangul (e.g. "키나")
  DO NOT attempt to romanize or phonetically convert it. Ever.
- If the source already provides a Latin spelling → use that exact spelling verbatim
- NEVER invent, guess, or phonetically render a Korean name into any Latin form
  ❌ 키나 → "Keyner"  /  ❌ 재현 → "Jae-hyeon"  /  ❌ 수인 → "Soo-inn"
  ✅ source says "키나" → write "키나"
  ✅ source says "Keena" → write "Keena"
  ✅ source says "Jaehyun" → write "Jaehyun"
...
export async function POST(req) {
  ...
  const ctx = await buildContext({ video, captions, news, artist, topic, type, hook, reason, keywords, articleText, articleSite, articleUrl })
